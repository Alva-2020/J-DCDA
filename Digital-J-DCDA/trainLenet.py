...
class Train():
    def __init__(self,class_num,batch_size,iters,learning_rate,keep_prob,param):
        ...
        # self.SourceData, self.SourceLabel=self.SourceData[0:100000],self.SourceLabel[0:100000]
    ...
    def TrainNet(self):
        ...
        a = 1
        self.inter_loss, self.intra_loss, self.centers_update_op = get_center_loss(Xs, labels, pseudo_feature_filter,
                                                                                   pseudo_label_filter, 0.5, 10)
        self.discriminative_loss = self.intra_loss + self.inter_loss
        self.discriminative_loss = self.discriminative_loss / (
                self.ClassNum * self.BatchSize + self.ClassNum * self.ClassNum)
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True))) as sess:
            ...
            for step in range(self.Iters):
                ...
                target_label, featuress, labelss = sess.run([pseudo_label_all, pseudo_feature_filter, pseudo_label_filter],
                                                            feed_dict={self.source_image: source_batch_x, self.source_label: source_batch_y,self.target_image: target_batch_x, self.Training_flag: False, self.W: W})
