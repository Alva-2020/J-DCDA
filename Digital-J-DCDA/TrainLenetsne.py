...
class Train():
    def __init__(self,class_num,batch_size,iters,learning_rate,keep_prob,param):
        ...
        self.SourceData,self.SourceLabel=load_mnist('mnist')
        self.TargetData, self.TargetLabel=load_mnistm('mnistm')
        self.TestData, self.TestLabel = load_mnistm('mnistm',split='test')
    ...
    def TrainNet(self):
        ...
        # Pseudo
        pseudo_label_all = tf.argmax(self.target_model.softmax_output, 1)
        pseudo_pro = tf.reduce_sum(
            tf.where(self.target_model.softmax_output < 0.8, x=tf.zeros_like(self.target_model.softmax_output),
                     y=self.target_model.softmax_output), 1)
        pseudo_pro_index = tf.where(pseudo_pro > 0)
        pseudo_label_filter = tf.gather(pseudo_label_all, pseudo_pro_index)
        pseudo_label_filter = tf.reshape(pseudo_label_filter, [-1])
        pseudo_feature_filter = tf.gather(self.target_model.fc4, pseudo_pro_index)
        pseudo_feature_filter = tf.reshape(pseudo_feature_filter, [-1, 64])

        # self.CalLoss()
        self.source_cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=self.source_label,
                                                                            logits=self.source_model.fc5)
        self.source_loss = tf.reduce_mean(self.source_cross_entropy)

        Xs = self.source_model.fc4
        labels = tf.argmax(self.source_label, 1)
        a = 1
        self.inter_loss, self.intra_loss, self.centers_update_op = get_center_loss(Xs, labels, pseudo_feature_filter, pseudo_label_filter, 0.5, 10)
        ...
        # self.domain_loss = self.CalDomainLoss(method="CORAL")
        Xs = self.source_model.fc4
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True))) as sess:
            ...
            for step in range(self.Iters):
                ...
                # sess.run(tf.nn.softmax(self.target_model.softmax_output),
                #          feed_dict={self.source_image: source_batch_x, self.target_image: target_batch_x, self.source_label: source_batch_y, self.Training_flag: True})
            ...
            # np.save("MNISTtoMNISTSOU.npy",savedata)
                if step % 2000 == 0:
                    # pass
                    savedata = np.array(lossData)
                    np.save("MNISTtoMNISTM_5.npy", savedata)
    ...
    #     plt.savefig('img03/c_%d.eps'%step,format="eps",dpi=1000,bbox_inches="tight")
    #
    ...
    #                 cmap=mpl.cm.jet)
    #
    #     plt.savefig('img03/d_%d.eps'%step,format="eps",dpi=1000,bbox_inches="tight")
    ...
    def conputeTSNE(self,step,source_images, target_images,source_labels,target_labels,sess):
        ...
        # plt.savefig('img2D1/%d.eps'%step)
        plt.savefig('img2D4/d_%d.eps' % step, format="eps", dpi=1000, bbox_inches="tight")
