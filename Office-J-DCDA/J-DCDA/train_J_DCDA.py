...
# tf.app.flags.DEFINE_integer('num_epochs',2, 'Number of epochs for training')
...
tf.app.flags.DEFINE_string('training_file', '../data/webcam.txt', 'Training dataset file')
tf.app.flags.DEFINE_string('val_file', '../data/amazon.txt', 'Validation dataset file')
...
def main(_):
    ...
    # Pseudo
    # target_pseudo = ResNetModel(target, is_training, depth=FLAGS.resnet_depth, num_classes=FLAGS.num_classes)
    ...
    pseudo_label_all=tf.argmax(target_model.prob, 1)
    pseudo_pro = tf.reduce_sum(tf.where(target_model.prob < 0.8, x=tf.zeros_like(target_model.prob), y=target_model.prob), 1)
    pseudo_pro_index=tf.where(pseudo_pro > 0)
    pseudo_label_filter=tf.gather(pseudo_label_all, pseudo_pro_index)
    print pseudo_label_filter
    # pseudo_label = tf.argmax(pseudo_label_filter)
    # print pseudo_label
    pseudo_feature_filter = tf.gather(target_model.avg_pool, pseudo_pro_index)
    print pseudo_feature_filter.shape
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True))) as sess:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            while step < train_batches_per_epoch:
                ...
                sess.run(tf.nn.softmax(target_model.prob), feed_dict={source: batch_xs,target:batch_xt, y: batch_ys, is_training: True})
